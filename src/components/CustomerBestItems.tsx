...
const CustomerBestItems: React.FC<CustomerBestItemsProps> = ({
  ...
  if (loading) {
    return <div className="py-8 text-center">Loading customer's favorite items...</div>;
  }

  return (
    <div className="space-y-4">
      <h2 className="text-lg font-medium">Customer's Favorite Items</h2>
      
      {bestItems.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          This customer hasn't ordered enough items to determine favorites yet
        </div>
      ) : (
            ...
            <Card key={product.id} className="p-4">
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="bg-primary/10 text-primary text-xs font-medium px-2 py-1 rounded-full flex items-center">
                  ...
                  Ordered {count} {count === 1 ? 'time' : 'times'}
                </div>
              </div>
